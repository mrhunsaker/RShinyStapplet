...
function addCountsTableRow()
{
    ...
    row.insertCell(0).innerHTML = '<TD><INPUT TYPE="text" CLASS="rowName" onChange="return deactivatePage(this)" PLACEHOLDER="Resp Grp ' + (table.rows.length - 3) + '" SIZE="15"></TD>';
    for (var i = 0; i < numDataColumns; i++)
        row.insertCell(i + 1).innerHTML = '<TD><INPUT TYPE="text" CLASS="data" onChange="return deactivatePage(this)" SIZE="15"></TD>';
...
function addCountsTableColumn()
{
    ...
    table.rows[1].insertCell(numDataColumns + 1).innerHTML = '<TD><INPUT TYPE="text" CLASS="colName" onChange="return deactivatePage(this)" PLACEHOLDER="Expl Grp ' + (numDataColumns + 1) + '" SIZE="15"></TD>';
    
    // In each of the next rows, add another data cell
    for (var i = 2; i < table.rows.length - 1; i++)
        table.rows[i].insertCell(numDataColumns + 1).innerHTML = '<TD><INPUT TYPE="text" CLASS="data" onChange="return deactivatePage(this)"  SIZE="15"></TD>';
...
function validateInputBuildData()
{
    ...
    if (UI.getProperty("selInputType", "value") == "counts")
    {
        ...
        for (var i = 1; i < rows[1].cells.length - 1; i++)               
            columnNames.push(UI.extractElementClassValue(rows[1].cells[i], "colName", true, true));
    ...
    if (!explanatoryName.length) explanatoryName = "Explanatory variable";
    if (!responseName.length) responseName = "Response variable";
...
function rebuildTable()
{
    ...
    table.rows[1].cells[1].getElementsByTagName("input")[0].placeholder = "Expl Grp 1";
    table.rows[1].cells[2].getElementsByTagName("input")[0].value = "";
    table.rows[1].cells[2].getElementsByTagName("input")[0].placeholder = "Expl Grp 2";
    while (table.rows[1].cells.length > 4) table.rows[1].deleteCell(3);
    table.rows[2].cells[0].getElementsByTagName("input")[0].value = "";
    table.rows[2].cells[0].getElementsByTagName("input")[0].placeholder = "Resp Grp 1";
    ...
    table.rows[3].cells[0].getElementsByTagName("input")[0].value = "";
    table.rows[3].cells[0].getElementsByTagName("input")[0].placeholder = "Resp Grp 2";
    table.rows[3].cells[1].getElementsByTagName("input")[0].value = "";
    table.rows[3].cells[2].getElementsByTagName("input")[0].value = "";
...
function updateSummaryStatistics()
{
    ...
    tableHTML += "<TR><TH ROWSPAN='" + (data.rowCategories.length + 2) + "'>"
    ...
    {   
        ...
        resultsCSV += "," + format.formatProportion(total) + "\r\n";
    }
...
function updateGraph()
{
	...
	graph.sideBySideBarChart(data, explanatoryName, (UI.getProperty("selSideBarGraphFreq", "value") == "rel"), responseName);
    }
    else
    {
        UI.setStyleProperty("spnSideBarGraphOptions", "display", "none");
        if (graphType == "stacked")
        	graph.stackedBarChart(data, explanatoryName, responseName);
        else // mosaic plot fallthrough
            graph.mosaicPlot(data, explanatoryName, responseName);
    }
}

function exportSVG(divId,desc)
{
    file.saveSVG(document.getElementById("svg_" + divId), "cat2v" + desc);
}
...
function updateInference()
{
    ...
    if (data.columnCategories.length == 2)
    {
		...
		var varname = "Simulated difference in sample " +
			(pref.getPreference(pref.keys.proportion.display_type) == "percent" ? "percentage" : "proportion");
