...
STAP.Utility =
{
    ...
    // Give theoretical Z's for a normal probability plot
    NPPZ: function(n)
    {
        var invs = [];
        for (var i = 0; i < n; i++)
            invs.push(jStat.normal.inv((i + 0.5)/n, 0, 1));
        return invs;
    },
    
    // Generate appropriate text for a 4-precision probability, using the approx symbol for p = 0 or p = 1 if needed
    // arrNoAsympCondFn is an array of callback functions for the calling applet to specify internal override conditions for the approx symbol
    probabilityHTML: function(sPre, p, arrNoAsympCondFn)
    {
        // Allow the user to specify the array using the internal property Utility.approxSymbolOverrideConditions instead of passing parameters
        arrNoAsympCondFn = arrNoAsympCondFn ||
            this.approxSymbolOverrideConditions;
        
        // There is no need for &asymp; if p is neither 0 nor 1
        var noAsymp = STAP.SafeNumber.roundWithinTolerance(p, 4) &&
                      STAP.SafeNumber.roundWithinTolerance(p - 1, 4);
        if (!noAsymp && arrNoAsympCondFn)
        {
            for (var i = 0; i < arrNoAsympCondFn.length; i++)
            {
                if (arrNoAsympCondFn[i]())
                {
                    noAsymp = true;
                    break;
                }   
        ...
        return sPre + (noAsymp ? " = " : " \u2248 ") +
            STAP.Format.formatProportion(p, 4);
    }
};
...
STAP.FileIO =
{
    ...
    saveSVG: function(svgElementObj, filename)
    {
        var serializer = new XMLSerializer();
        var source = serializer.serializeToString(svgElementObj);
        ...
        downloadLink.download = (filename ? filename : "plot") + ".svg";
...
{
    keyBase: "STAP.",
    dataTransferBase: "STAP.transfer.",
    ...
    // commit page transfer data to sessionStorage temporarily
    // using sessionStorage so that if the tab is closed, the data goes away anyway even if we're not fully able to do housekeeping
    setPageTransferData: function(keys, dataArrArr, varname)
    {
        window.sessionStorage.setItem("STAP.transfer_master.keys", JSON.stringify(keys));
        if (varname)
            window.sessionStorage.setItem("STAP.transfer_master.variableName", varname);
        for (var i = 0; i < keys.length; i++)
        {
            window.sessionStorage.setItem(this.dataTransferBase + keys[i],
                JSON.stringify(dataArrArr[i]));
        }
    },
    
    // pull page transfer data into a returned data object, clearing out sessionStorage as you go
    getPageTransferData: function()
    {
        var keys = JSON.parse(window.sessionStorage.getItem("STAP.transfer_master.keys"));
        if (!Array.isArray(keys)) return null;

        var retval = { keys: keys.slice(0), dataMap: {} };
        for (var i = 0; i < keys.length; i++)
        {
            retval.dataMap[keys[i]] = JSON.parse(window.sessionStorage.getItem(this.dataTransferBase + keys[i]));
            window.sessionStorage.removeItem(this.dataTransferBase + keys[i]);
        }
        window.sessionStorage.removeItem("STAP.transfer_master.keys");
        
        var varname = window.sessionStorage.getItem("STAP.transfer_master.variableName");
        if (varname)
        {
            retval.variableName = varname;
            window.sessionStorage.removeItem("STAP.transfer_master.variableName");
        }
        return retval;
    }    
};
...
STAP.Preferences =
{
    ...
    defaults:
        { number:
          ...
          { dot_radius: "2",
            ...
            select_color: "#660000",
            font_size: "14",
            font_face: "AvenirNextProRegular, sans-serif"
          }
        },
    ...
    closePreferencePage: function()
    {
        ...
        if (this.preferencePageCloseCallback)
            this.preferencePageCloseCallback();
    }
};
...
STAP.SafeNumber = 
{
    ...
    // https://stackoverflow.com/questions/40293348/does-the-number-0-exists-in-javascript#:~:text=Floating%20point%20numbers%20have%20a,rid%20of%20the%20minus%20sign.
    roundWithinTolerance: function(val, tol)
    {
        if (typeof(tol) === "undefined") tol = STAP.Preferences.getZeroTolerance();
        var retval = parseFloat(val.toFixed(tol));
        if (val < 0 && Number(retval) === 0) // check for negative zero
            return 0;
        else
            return retval;
    },
    ...
    // Convenience function for checking betweenness
    // Assumed inclusive unless bExcl is specified
    isBetween: function(x, a, b, bExcl, tol)
    {
        var safenum = STAP.SafeNumber;
        var comp1 = safenum.compareToWithinTolerance(a, x, tol);
        var comp2 = safenum.compareToWithinTolerance(x, b, tol);
        return (bExcl ? comp1 < 0 && comp2 < 0 : comp1 <= 0 && comp2 <= 0);
    },
...
// set an fnCountHTML callback EACH TIME the dotplot updates if you want a message in the count other than the default counting
STAP.SVGGraph.prototype.dotplot = function(data, xAttr, caseAttr, bAllowCount, bIsRelative, forceDomainArr, forceDotSize, fillColor)
{
	var self = this;
	var dataMap = {};
	...
	if (bAllowCount)
	{
		...
		this.selectionCallback = function(finalAttributes) {
			if (self.currentData)
			{
				...
				var countHTML = "<BR>" + (self.fnCountHTML ? self.fnCountHTML() : "There " +
					(sel == 1 ? "is " : "are ") + sel + (sel == 1 ? " dot" : " dots") + " (" +
				 	STAP.Format.formatPercent(sel / self.currentData.length)
					+ ") in the selected region.");
				self.fnCountHTML = null;
				document.getElementById(countSpanID).innerHTML = countHTML;
			}	
		};
...
STAP.SVGGraph.prototype.binomialHistogram = function(n, p, fnResultsCallback)
{
	var self = this;
	...
	this.clearGraph();
    this.resultsCallback = fnResultsCallback;
	...
	this.selectionCallback = function(finalAttributes) {
		if (self.currentData)
		{
            ...
            if (self.resultsCallback)
                self.resultsCallback(min, max, sum);
		}
	};

	this.unselectionCallback = function() {
	    if (self.resultsCallback)
	        self.resultsCallback(-1, -1, 0);
	};

	this.setSelectionRectangleEnabled(true);
};

STAP.SVGGraph.prototype.discreteProbabilityHistogram = function(data, xAttr, bHideYAxis)
{
	var self = this;
	var yAttr = "Probability";
    ...
    // If we force the ticks as below, no need to nice-ify the domain
    ...
    /* OPTION 1
    var xTicks = [];
    for (var i = 0; i < data.length; i++) xTicks.push(data[i].x);
	STAP.SafeNumber.cleanArray(xTicks);
	xAxis.tickValues(xTicks); */
	
	/* OPTION 2 */
    xAxis.ticks(Math.min(data.length, 5));
	...
	var numtext = (STAP.SafeNumber.isZeroWithinTolerance(parseFloat(STAP.Format.formatProportion(b.height))) ? "&asymp; 0"			
	  ...
	  var tooltip_text = "P(X = " + STAP.Format.formatNumber((b.left + b.right)/2) + ") " + numtext;
	  self.tooltip.html(tooltip_text)
	       .style("left", (d3.event.pageX + 5) + "px")
	       .style("top", (d3.event.pageY - 28) + "px");
	})
...
STAP.SVGGraph.prototype.boxplot = function(data, xAttr, matchGraph)
{
	var self = this;
	...
	xScale.domain(matchGraph ? matchGraph.xScale.domain() :
	    [fns[0] - buffer, fns[4] + buffer]);
	yScale.domain(matchGraph ? [2,4] : [1,5]);
	...
	xAxis.tickValues(xTicks);
	if (matchGraph)
        this.adjustWidthForLabels(matchGraph.xAxis, matchGraph.yAxis);
    else
    	this.adjustWidthForLabels(xAxis);
...
STAP.SVGGraph.prototype.barChart = function(data, xAttr, bRelative, yDisplayAttr, forceNumYTicks)
{
	this.clearGraph();
	
	xAttr = xAttr || "Category";
	var yAttr = (bRelative ? "Relative Frequency" : "Frequency");
    yDisplayAttr = yDisplayAttr || yAttr;
    ...
    if (forceNumYTicks) yAxis.ticks(forceNumYTicks);
	...
	var verticaloffset = parseInt(pref.getPreference(pref.keys.drawing.font_size)) / 2 - 2;
	  this.svg.append("g")
	      .attr("class", "axis axis--y")
	      .call(yAxis)
	    .append("text")
    ...
    var deschtml = "The figure shows a bar chart. The vertical axis is labeled " + yDisplayAttr + " and is labeled from 0 to " + yAxis.tickFormat()(y.domain()[1]);
    if (y.ticks().length > 1)
...
STAP.SVGGraph.prototype.stackedBarChart = function(data, xAttr, legAttr)
{
	...
	var legend = this.svg.append("g")
		.attr("transform", "translate(" + 2.1 * this.width / 3 + ", 5)");
    ...
    {
        deschtml += "The bar for Group " + graphingData[i].Group + " is divided at the following percents: Category " + graphingData[i].Category + ", ";
...
STAP.SVGGraph.prototype.mosaicPlot = function(data, xAttr, legAttr)
{
	...
	var self = this;
	this.currentData = data;
    this.width = 1.52 * this.height; // 1.52 is empirical for appx square plot
	var x = d3.scale.linear()
	        .rangeRound([0, this.width * 2 / 3]),
	    y = d3.scale.linear()
			.rangeRound([this.height, 0]), // -20 is room for top axis
	    xAxis = d3.svg.axis().outerTickSize(0).scale(x).orient("bottom"),
	    yAxis = d3.svg.axis().outerTickSize(0).scale(y).orient("left"),
	    tAxis = d3.svg.axis().outerTickSize(0).scale(x).orient("top");

	yAxis.tickFormat(STAP.Format.d3ProportionFormatter());
	tAxis.tickFormat(STAP.Format.d3ProportionFormatter());
	tAxis.ticks(5);
	var color = d3.scale.category20();
...
STAP.SVGGraph.prototype.scatterplot = function(data, xAttr, yAttr, caseAttr, dotRadius, forceXDomainArr, forceYDomainArr, groupName, forceXTicks, forceYTicks)
{
	...
	var xTicks = (forceXTicks ? xAxis.scale().ticks(forceXTicks) : xAxis.scale().ticks());
	STAP.SafeNumber.cleanArray(xTicks);
	var yTicks = (forceYTicks ? yAxis.scale().ticks(forceYTicks) : yAxis.scale().ticks());
	STAP.SafeNumber.cleanArray(yTicks);
	xAxis.tickValues(xTicks);
	yAxis.tickValues(yTicks);
	if (groupName && groupName.length)
    	this.adjustWidthForLabels(xAxis, yAxis, [groupName]);
    else
    	this.adjustWidthForLabels(xAxis, yAxis);
...
STAP.SVGGraph.prototype.plotTopCurve = function(fn, n, color, bDashed, bNoClear, bNormalize)
{
	var self = this;
    ...
    if (bNormalize)
    {
        ...
        const dMax = this.yScale.domain()[1];
        const dMin = this.yScale.domain()[0];
        var normFn = function(x) {
            return dMin + (x - min) / (max - min) * (dMax - dMin);
        };
        for (var i = 0; i < data.length; i++)
            data[i].y = normFn(data[i].y);
    }
...
STAP.SVGGraph.prototype.uniformDiagram = function(min, max, left, right, bBetween)
{
	this.clearGraph();

    var fn = function(x) {
        return (x <= max && min <= x ? 1 : 0);
    };
...
// negative lean -- orient label to the left of the arrow
// zero lean -- orient label directly above or below arrow
// positive lean -- orient label to the right of the arrow
// yScale and xScale must be set
STAP.SVGGraph.prototype.addCurveLabel = function(text, worldX, yFn, leanX, bBelowAxis, desiredHeight)
{
    const arrowBuffer = 10;
    desiredHeight = desiredHeight || 25;

    var x2 = this.xScale(worldX);
    if (x2 < 0) x2 = 0;
    
    var maxX = this.xScale(this.xScale.domain()[1]);
    if (x2 > maxX) x2 = maxX;
    var x1 = x2 + leanX;

    var yWorldAnchor = (bBelowAxis ? 0 : yFn(worldX));
    var y2 = this.yScale(yWorldAnchor);
    var y1 = this.yScale(yWorldAnchor) + (bBelowAxis ? 1 : -1) * desiredHeight;
    if (!bBelowAxis && y1 < 0)
    {
        y1 = 0;
        y2 = desiredHeight;
    ...
    if (!bBelowAxis && (y2 - y1) < 10)
        y2 = y1 + 10;

    // Return the label group appended
	var g = this.svg.append("g");
	var lbl = g.append("text")
	    .style("opacity", 0)
		.style("font-family", STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_face))
		.style("font-size", STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_size + "px"))
        .style("text-anchor", (leanX > 0
            ? "start"
            : (leanX < 0 ? "end" : "middle")
        ))
        .attr("x", x1 + (leanX > 0 ? -arrowBuffer : (leanX < 0 ? arrowBuffer : 0)))
        .attr("y", y1 + (bBelowAxis ? 10 : -2))
        .text(text);
    
    if (!bBelowAxis)
    {
        var bbox = lbl[0][0].getBBox();
        if (bbox.width + bbox.x > this.width)
        {
            lbl.attr("x", x2 - (bbox.width + bbox.x - this.width));
            x1 = Math.min(x1, this.width - bbox.width * 0.5);
        }
        else if (bbox.x < 0)
        {
            lbl.attr("x", x2 - bbox.x)
            x1 = Math.max(x1, bbox.width * 0.5);
        }            
    }
    lbl.style("opacity", 1);
...
STAP.SVGGraph.prototype.normalDiagram = function(mean, stdev, left, right, bBetween, bLabels)
{
	this.clearGraph();
    ...
    var bIsStandard = (STAP.SafeNumber.isZeroWithinTolerance(mean) && STAP.SafeNumber.isZeroWithinTolerance(stdev - 1));
	...
	if (plotSelected)
	{
		...
		var midCdf = function(x1, x2)
		{
		    var rCdf = jStat.normal.cdf(x2, mean, stdev);
		    var lCdf = jStat.normal.cdf(x1, mean, stdev);
            return jStat.normal.inv(lCdf + 0.5 * (rCdf - lCdf), mean, stdev);
		}
		...
		if (bBetween)
		{
			...
			if (bLabels)
			{
			    // In general, shoot to put the area label arrow at the value that splits the cdf of the labeled region into 2
			    var labelX = midCdf(left, right);
                var leanSize = 10;
                
                // Do not let the label point get too close to the center of the plot; if the shaded area is very narrow, use an edge value
                var buffer = 0.75 * stdev;
                if ((mean - buffer) <= labelX && labelX <= mean)
                {
                    labelX = mean - buffer;
                    if (labelX < left)
                        labelX = left;
                }
                else if (mean <= labelX && labelX <= (mean + buffer))
                {
                    labelX = mean + buffer;
                    if (labelX > right)
                        labelX = right;
			    ...
			    // A label anchor point to the left of the mean should lean left and a boundary point on the right should lean right
			    var dispArea = (origLeft === null ? (origRight === null ? 1 : jStat.normal.cdf(origRight, mean, stdev)) : (origRight === null ? 1 - jStat.normal.cdf(origLeft, mean, stdev) : jStat.normal.cdf(origRight, mean, stdev) - jStat.normal.cdf(origLeft, mean, stdev)));
			        
	            this.addCurveLabel(STAP.Utility.probabilityHTML("Area", dispArea), labelX, fn, (labelX > mean ? 10 : -10));
                    
                var boundsClose = (origLeft !== null && origRight !== null && ((right - left) < 0.1 * (r4s - l4s)));
                if (origLeft !== null && l4s <= origLeft && origLeft <= r4s)
                    this.addCurveLabel((bIsStandard ? "z" : "x") + " = " + STAP.Format.formatNumber(origLeft), origLeft, fn,
                        (boundsClose ? -10 : 0), true);
                if (origRight !== null && l4s <= origRight && origRight <= r4s)
                    this.addCurveLabel((bIsStandard ? "z" : "x") + " = " + STAP.Format.formatNumber(origRight), origRight, fn,
                        (boundsClose ? 10 : 0), true);
			}
		}
		else
		{
			...
			if (bLabels)
			{
			    if (origLeft > l4s)
			    {
		            var labelX = midCdf(l4s, left);
    			    var bLabelLeanRight = (left > mean);
                    this.addCurveLabel(STAP.Utility.probabilityHTML("Area", jStat.normal.cdf(origLeft, mean, stdev)), labelX, fn, (bLabelLeanRight ? 5 : -5));
                    this.addCurveLabel((bIsStandard ? "z" : "x") + " = " + STAP.Format.formatNumber(left), left, fn, 0, true);
			    }
			    if (origRight < r4s)
			    {
			        var labelX = midCdf(right, r4s);
    			    var bLabelLeanRight = (right > mean);
                    this.addCurveLabel(STAP.Utility.probabilityHTML("Area", 1 - jStat.normal.cdf(origRight, mean, stdev)), labelX, fn, (bLabelLeanRight ? 5 : -5));
                    this.addCurveLabel((bIsStandard ? "z" : "x") + " = " + STAP.Format.formatNumber(right), right, fn, 0, true);
			    }
			}
		}
	}
    ...
    if (bLabels)
    {
        var fontsize = parseFloat(STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_size));
        if (bIsStandard)
        {
            this.svg.append('text')
        		.style("font-family", STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_face))
        ...
        else
        {
            this.svg.append('text')
                ...
                .attr("x", this.width)
                .attr("y", fontsize)
                .text("N(" + STAP.Format.formatNumber(mean) + ", "
                           + STAP.Format.formatNumber(stdev) + ")");            
        }
    }
...
STAP.SVGGraph.prototype.powerDiagramMean = function(mean0, mean1, popSD, n, alpha, altType, plotType, bShowReject)
{
    return this.powerDiagramCommon(mean0, popSD/Math.sqrt(n), mean1, popSD/Math.sqrt(n), alpha, altType, plotType, bShowReject, "x&#772;");
};

STAP.SVGGraph.prototype.powerDiagramProportion = function(prop0, prop1, n, alpha, altType, plotType, bShowReject)
{
    var stdev = function(n, p) {
        return Math.sqrt(p * (1-p) / n);
    }
    return this.powerDiagramCommon(prop0, stdev(n, prop0), prop1,
        stdev(n, prop1), alpha, altType, plotType, bShowReject, "p&#770;"); 
};

// plotType: 1 = alpha, 2 = beta, 3 = power
// returns area plotted under curve 1 -- ignore for alpha plot
STAP.SVGGraph.prototype.powerDiagramCommon = function(mean0, stdev0, mean1, stdev1, alpha, altType, plotType, bShowReject, rejectSymbol)
{
    var self = this;
	this.clearGraph();

	var fn0 = STAP.Utility.normalCurve(mean0, stdev0);
	var fn1 = STAP.Utility.normalCurve(mean1, stdev1);
    ...
    var stdev = Math.max(stdev0, stdev1);
	var l4s = Math.min(mean0, mean1) - 3.5 * stdev;
	var r4s = Math.max(mean0, mean1) + 3.5 * stdev;
	
	xScale.domain([l4s, r4s]);
	yScale.domain([0, Math.max(fn0(mean0), fn1(mean1))]);
    ...
    var isBeta = (plotType == 2);
    var isOneSidedBeta = (isBeta && altType !== stat.TWO_SIDED);
    var leftValid = (altType !== stat.ONE_SIDED_GT);
    var rightValid = (altType !== stat.ONE_SIDED_LT);

	var rejectXL = (leftValid ? 
	    jStat.normal.inv((altType == stat.TWO_SIDED ? alpha/2 : alpha), 0, 1) * stdev0 + mean0 : null);
	var rejectXR = (rightValid ?
	    jStat.normal.inv(1 - (altType == stat.TWO_SIDED ? alpha/2 : alpha), 0, 1) * stdev0 + mean0 : null);

    var power = (leftValid ? jStat.normal.cdf(rejectXL, mean1, stdev1) : 0) +
        (rightValid ? 1 - jStat.normal.cdf(rejectXR, mean1, stdev1) : 0);

    var rangeMapFn = (plotType == 1 ? fn0 : fn1);
    var retval = 0;
    var maxY = yScale(yScale.domain()[1]);

    var appendRect = function(worldLeft, worldRight, fillColor, tooltipText)
    {
        if (!fillColor) fillColor = "#99FF99";
        return self.svg.append("rect")
	      .attr("x", xScale(worldLeft))
	      .attr("y", maxY)
	      .attr("width", xScale(worldRight) - xScale(worldLeft))
	      .attr("height", self.height - maxY)
	      .style("fill", fillColor)
	      .style("stroke-width", 0)
            .on("mouseover", function(b) {
            	  self.tooltip.transition()
    ...
    var appendArea = function(data)
    {
        var toolhtml = (plotType == 1 ? "&alpha; = " + format.formatProportion(alpha) :
                (plotType == 2 ? "&beta; = " + format.formatProportion(1 - power) : "Power = " + format.formatProportion(power))
        );
	...
	{
	    if (leftValid)
	        appendRect(l4s, rejectXL, "#99FF99", "Reject " + rejectSymbol
	            + " &lt; " + format.formatNumber(rejectXL));
	    if (rightValid)
	        appendRect(rejectXR, r4s, "#99FF99", "Reject " + rejectSymbol
	            + " &gt; " + format.formatNumber(rejectXR));
	}
    
	if (plotType == 2 && leftValid && rightValid) // beta, two-sided
	{
		appendArea(this.fnRangeMap(rangeMapFn, rejectXL, rejectXR, 80));

		retval = jStat.normal.cdf(rejectXR, mean1, stdev1) -
            		jStat.normal.cdf(rejectXL, mean1, stdev1);
	}
	else
	{
	    if (rejectXL)
	    {
	        if (isOneSidedBeta)
	        {
        		appendArea(this.fnRangeMap(rangeMapFn, rejectXL, r4s, 80));
        		retval = 1 - jStat.normal.cdf(rejectXL, mean1, stdev1);
	        }
	        else
	        {
        		appendArea(this.fnRangeMap(rangeMapFn, l4s, rejectXL, 80));
        		retval += jStat.normal.cdf(rejectXL, mean1, stdev1);
	        }
	    }
	    if (rejectXR)
	    {
	        if (isOneSidedBeta)
	        {
        		appendArea(this.fnRangeMap(rangeMapFn, l4s, rejectXR, 80));
        		retval = jStat.normal.cdf(rejectXR, mean1, stdev1);
	        }
	        else
	        {
        		appendArea(this.fnRangeMap(rangeMapFn, rejectXR, r4s, 80));
        		retval += (1 - jStat.normal.cdf(rejectXR, mean1, stdev1));
	        }
    	}
	}
	...
	// Finally, draw the Gaussians
	// fn, n, color, bDashed, bNoClear
	this.plotTopCurve(fn0, 80, null, true, true);
    this.plotTopCurve(fn1, 80, null, false, true);
...
STAP.SVGGraph.prototype.TDiagram = function(df, left, right, bBetween, bPlotNormal, bLabels)
{
	this.clearGraph();
	...
	if (plotSelected)
	{
		...
		var midCdf = function(t1, t2)
		{
		    var rCdf = jStat.studentt.cdf(t2, df);
		    var lCdf = jStat.studentt.cdf(t1, df);
            return jStat.studentt.inv(lCdf + 0.5 * (rCdf - lCdf), df);
		}
		...
		if (bBetween)
		{
			...
			if (bLabels)
			{
                ...
                // Do not let the label point get too close to the center of the plot; if the shaded area is very narrow, use an edge value
                var buffer = 0.0833 * (r4s - l4s), mean = 0;
                if ((mean - buffer) <= labelX && labelX <= mean)
                {
                    labelX = mean - buffer;
                    if (labelX < left)
			    ...
			    // A label anchor point to the left of the mean should lean left and a boundary point on the right should lean right
			    var dispArea = (origLeft === null ? (origRight === null ? 1 : jStat.studentt.cdf(origRight, df)) : (origRight === null ? 1 - jStat.studentt.cdf(origLeft, df) : jStat.studentt.cdf(origRight, df) - jStat.studentt.cdf(origLeft, df)));
			        
	            this.addCurveLabel(STAP.Utility.probabilityHTML("Area", dispArea), labelX, fn, (labelX > mean ? 10 : -10));
                    
                var boundsClose = (origLeft !== null && origRight !== null && ((right - left) < 0.1 * (r4s - l4s)));
                if (origLeft !== null && l4s <= origLeft && origLeft <= r4s)
                    this.addCurveLabel("t = " + STAP.Format.formatNumber(origLeft), origLeft, fn,
                        (boundsClose ? -10 : 0), true);
                if (origRight !== null && l4s <= origRight && origRight <= r4s)
                    this.addCurveLabel("t = " + STAP.Format.formatNumber(origRight), origRight, fn,
                        (boundsClose ? 10 : 0), true);                
			}
		}
		else
		{
			...
			if (bLabels)
			{
			    if (origLeft > l4s)
			    {
		            var labelX = midCdf(l4s, left);
    			    var bLabelLeanRight = (left > mean);
                    this.addCurveLabel(STAP.Utility.probabilityHTML("Area", jStat.studentt.cdf(origLeft, df)), labelX, fn, (bLabelLeanRight ? 5 : -5));
                    this.addCurveLabel("t = " + STAP.Format.formatNumber(left), left, fn, 0, true);
			    }
			    if (origRight < r4s)
			    {
			        var labelX = midCdf(right, r4s);
    			    var bLabelLeanRight = (right > mean);
                    this.addCurveLabel(STAP.Utility.probabilityHTML("Area", 1 - jStat.studentt.cdf(origRight, df)), labelX, fn, (bLabelLeanRight ? 5 : -5));
                    this.addCurveLabel("t = " + STAP.Format.formatNumber(right), right, fn, 0, true);
			    }
			}
		}
	}
    ...
    if (bLabels)
    {
        ...
        this.svg.append('text')
            ...
            .attr("x", this.width)
            .attr("y", fontsize)
            .text("t(" + STAP.Format.formatNumber(df) + ")");            
    }
    ...
    if (bPlotNormal)
        this.plotTopCurve(nfn, 100, "#CC33CC", true, true);

	STAP.Preferences.writeCSSRules();
};

// chi-square diagram
STAP.SVGGraph.prototype.X2Diagram = function(df, left, right, bBetween, bLabels)
{
	this.clearGraph();
    ...
    if ((df - 2) > (farthest / 2)) farthest = (df - 2) * 2;
	...
	if (plotSelected)
	{
		...
		var splitCdf = function(x1, x2)
		{
		    var rCdf = jStat.chisquare.cdf(x2, df);
		    var lCdf = jStat.chisquare.cdf(x1, df);
            return jStat.chisquare.inv(lCdf + 0.7 * (rCdf - lCdf), df);
		};

		if (bBetween)
		{
			var seldata = this.fnRangeMap(fn, left, right, 150);
			...
			if (bLabels)
			{
			    var dispArea = (origLeft === null ? (origRight === null ? 1 : jStat.chisquare.cdf(origRight, df)) : (origRight === null ? 1 - jStat.chisquare.cdf(origLeft, df) : jStat.chisquare.cdf(origRight, df) - jStat.chisquare.cdf(origLeft, df)));
			    
			    // Because the data is skewed, try to split the cdf at about 70%
			    var labelX = splitCdf(left, right);

			    // If the max is less than 1/3 of the way across the diagram, keep the lean but make sure the arrow points at the actual area
			    if ((df - 2) < (farthest / 3))
			    {
			        // The default behavior often breaks when the region is to the far left and the max is to the far left; label this side manually in this case
    			    if (labelX < (df - 2))
    			    {
    			        var fontsize = parseFloat(STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_size));
                    	var g = this.svg.append("g");
                    	g.append("text")
                            ...
                            .attr("x", xScale(right) + 20)
                            .attr("y", yScale(0.05 * max))
                            .text(STAP.Utility.probabilityHTML("Area", dispArea));
                        
                        g.append("line")
                            .attr("x1", xScale(right) + 18)
                            .attr("y1", yScale(0.05 * max) - fontsize / 4)
                            .attr("x2", xScale(right))
                            .attr("y2", Math.max(yScale(0.15 * max),
                                                 yScale(fn(right))))
                            .style("stroke", "#000000")
                            .attr("marker-end", "url(#triangle)");
    			    }
    			    ...
    			    {
        	            this.addCurveLabel(STAP.Utility.probabilityHTML("Area", dispArea), labelX, fn, 10);
    			    }
                }
                else
                {
                    // Do not let the label point get too close to the max of the plot; if the shaded area is very narrow, use an edge value
                    var buffer = 0.0833 * farthest, peak = df - 2;
                    if ((peak - buffer) <= labelX && labelX <= peak)
                    {
                        labelX = peak - buffer;
                        if (labelX < left)
                            labelX = left;
                    }
                    else if (peak <= labelX && labelX <= (peak + buffer))
                    {
                        labelX = peak + buffer;
                        if (labelX > right)
                            labelX = right;
                    }
    	            this.addCurveLabel(STAP.Utility.probabilityHTML("Area", dispArea), labelX, fn, (labelX > peak ? 10 : -10));
                }    
                    
                var boundsClose = (origLeft !== null && origRight !== null && ((right - left) < (0.1 * farthest)));
                if (origLeft !== null && 0 <= origLeft && origLeft <= farthest)
                    this.addCurveLabel("\u03c7\u00b2 = " + STAP.Format.formatNumber(origLeft), origLeft, fn,
                        (boundsClose ? -10 : 0), true);
                if (origRight !== null && 0 <= origRight && origRight <= farthest)
                    this.addCurveLabel("\u03c7\u00b2 = " + STAP.Format.formatNumber(origRight), origRight, fn,
                        (boundsClose ? 10 : 0), true);
			}
		}
		else // The diagram will not be labeled if this option is taken
		{
    ...
    if (bLabels)
    {
        ...
        this.svg.append('text')
            ...
            .attr("x", this.width)
            .attr("y", fontsize)
            .text("\u03c7\u00b2(" + STAP.Format.formatNumber(df) + ")");
    }
...
// Assumes Decimal library is loaded
STAP.SVGGraph.prototype.FDiagram = function(numdf, denomdf, left, right, bBetween, bLabels)
{
	this.clearGraph();
    ...
    /* 	old broken library pdf
	var fn = (numdf > 2 ? function(x) {
	    var ret = jStat.centralF.pdf(x, numdf, denomdf);
	    return (isNaN(ret) ? 0 : ret); } :
        function(x)
        { var ret = Math.min(3, jStat.centralF.pdf(x, numdf, denomdf));
            return (isNaN(ret) ? 0 : ret);    
        });
    */
    var fn = function(x) {
        var pdf = Decimal.pow(numdf / denomdf, numdf / 2).div(jStat.betafn(numdf / 2, denomdf / 2)).mul(Decimal.pow(x, numdf / 2 - 1)).mul(Decimal.pow(1 + numdf / denomdf * x, -(numdf + denomdf) / 2)).toNumber();
        return Math.min(pdf, 10);
    };
    ...
    var dfprod = numdf * denomdf;
    var farthest = 5 + Math.max(0, (101 - dfprod)/20);
    var maxX = ((numdf < 3) ? (farthest / 100) : (numdf - 2)/numdf*denomdf/(denomdf + 2));
    var max = fn(maxX);
	xScale.domain([0, farthest]);
	yScale.domain([0, max]);
...
/*		var splitCdf = function(x1, x2)
		{
		    var rCdf = fn(x2);
		    var lCdf = fn(x1);
            return jStat.centralF.inv(lCdf + 0.7 * (rCdf - lCdf), numdf, denomdf);
		};
*/
        var rangeMapSize = Math.round((right - left) / farthest * 110);
				
		if (bBetween)
		{
			var seldata = this.fnRangeMap(fn, left, right, rangeMapSize);
			...
			if (bLabels)
			{
			    var dispArea = (origLeft === null ? (origRight === null ? 1 : jStat.centralF.cdf(origRight, numdf, denomdf)) : (origRight === null ? 1 - jStat.centralF.cdf(origLeft, numdf, denomdf) : jStat.centralF.cdf(origRight, numdf, denomdf) - jStat.centralF.cdf(origLeft, numdf, denomdf)));
			    
			    // This is even harder, so just go 50% between left and right
			    var labelX = (left + right) / 2;
                ...
                {
                    // Do not let the label point get too close to the max of the plot; if the shaded area is very narrow, use an edge value
                    var buffer = 0.0833 * farthest;
                    if ((maxX - buffer) <= labelX && labelX <= maxX)
                    {
                        labelX = maxX - buffer;
                        if (labelX < left)
                            labelX = left;
                    }
                    else if (maxX <= labelX && labelX <= (maxX + buffer))
                    {
                        labelX = maxX + buffer;
                        if (labelX > right)
                            labelX = right;
                    }
    	            this.addCurveLabel(STAP.Utility.probabilityHTML("Area", dispArea), labelX, fn, (labelX > maxX ? 10 : -10));
                }    
                    
                var boundsClose = (origLeft !== null && origRight !== null && ((right - left) < (0.1 * farthest)));
                if (origLeft !== null && 0 <= origLeft && origLeft <= farthest)
                    this.addCurveLabel("F = " + STAP.Format.formatNumber(origLeft), origLeft, fn,
                        (boundsClose ? -10 : 0), true);
                if (origRight !== null && 0 <= origRight && origRight <= farthest)
                    this.addCurveLabel("F = " + STAP.Format.formatNumber(origRight), origRight, fn,
                        (boundsClose ? 10 : 0), true);
			}
		}
		else // the plot will not be labeled in this case
		{
			var seldata1 = this.fnRangeMap(fn, 0, left, rangeMapSize);
			var seldata2 = this.fnRangeMap(fn, right, farthest, rangeMapSize);
    ...
    if (bLabels)
    {
        ...
        this.svg.append('text')
            ...
            .attr("x", this.width)
            .attr("y", fontsize)
            .text("F(" + STAP.Format.formatNumber(numdf) + ", " + STAP.Format.formatNumber(denomdf) + ")");
    }

	// Finally, draw the F curve
	this.plotTopCurve(fn, 100);

	STAP.Preferences.writeCSSRules();
};

// bimodal diagram made of N(a, b) "copied" over by c units
STAP.SVGGraph.prototype.bimodalDiagram = function(a, b, c)
{
	this.clearGraph();

    // https://www.milefoot.com/math/stat/rv-sums.htm
    // If X is N(a, b) and Y is discrete 50% 0 and 50% x, then Z = X + Y and
    // pdfz(z) = integral from -inf to inf of pdfx(x) pdfy(z - x) dx
    // Convolution calculated on wolfram alpha...
    var fn = function(x)
    {
        return Math.exp(-(x*x-2*a*x+(a+c)*(a+c))/(2*b*b))*
            (Math.exp(c*x/(b*b))+Math.exp(c*(c+2*a)/(2*b*b)))/(2*Math.sqrt(2*Math.PI)*b);
    };
    ...
    var left = Math.min(a, a + c) - 4 * b;
    var right = Math.max(a, a + c) + 4 * b;
	xScale.domain([left, right]);
	yScale.domain([0, fn(a)]);
	...
	this.svg.append('path')
		.attr('class', 'area')
		.datum(this.fnRangeMap(fn, left, right, 60))
		.attr('d', area);			
	...
	// Finally, draw the chi-square curve
	this.plotTopCurve(fn, 60);

	STAP.Preferences.writeCSSRules();
};

// arr expects intervals, not items containing intervals
STAP.SVGGraph.prototype.confidenceIntervalPlot = function(arr, param, forceDomainArr)
{
	this.clearGraph();

    var dotSize = ((this.height / arr.length / 2) < 4 ? 2 : 4);
 
    var plotData = arr.slice(0);
    for (var i = 0; i < plotData.length; i++)
        plotData[i].plotNum = i + 1;
    ...
    if (forceDomainArr)
        xScale.domain(forceDomainArr);
    else
    {
        var lowerBoundMin = arr.reduce(function(prev, current) {
                return ((prev.lowerBound < current.lowerBound) ? prev : current);
            }).lowerBound;
        var upperBoundMax = arr.reduce(function(prev, current) {
                return ((prev.upperBound > current.upperBound) ? prev : current);
            }).upperBound;
    
        var leftArm = Math.abs(param - lowerBoundMin);
        var rightArm = Math.abs(upperBoundMax - param);
        var reach = 1.1 * Math.max(leftArm, rightArm);
    	xScale.domain([param - reach, param + reach]);
    }
    	
	yScale.domain([0, arr.length + 1]);
    ...
    // plot parameter
	this.svg.append("line")
		.attr("x1", xScale(param))
		.attr("y1", 0)
		.attr("x2", xScale(param))
		.attr("y2", this.height)
		.attr("stroke-dasharray", "10,10")
        .style("stroke", "#000000");

    var orderColor = function(a, b, c) {
        return ((a <= b && b <= c) ? "#009900" : "#990000");
    }

    var orderText = function(a, b, c) {
        return ((a <= b && b <= c) ? "[hit]" : "[miss]");
    }
    ...
    this.svg.selectAll("circle")
        ...
        .attr("r", dotSize)
        .attr("cx", function(d) {
            return xScale(0.5 * (d.lowerBound + d.upperBound));
        })
        .attr("cy", function(d) {
            return yScale(d.plotNum);
        })
        .style("fill", function(d) {
            return orderColor(d.lowerBound, param, d.upperBound);
        })
        ...
        .on("mouseover", function(d) {
        	  self.tooltip.transition()
        	       ...
        	       .style("opacity", .9);
        	  self.tooltip.html(
                "(" + format.formatNumber(d.lowerBound) + ", " + format.formatNumber(d.upperBound) + ") " + orderText(d.lowerBound, param, d.upperBound)
        	  )
    	       .style("left", (d3.event.pageX + 10) + "px")
    	       .style("top", (d3.event.pageY - 10) + "px");
        	})
    ...
    this.svg.selectAll(".CIline")
        .data(plotData)
        .enter()
        .append("line")
        .attr("class", "CIline")
		.attr("x1", function(d) { return xScale(d.lowerBound); })
		.attr("y1", function(d) { return yScale(d.plotNum); })
		.attr("x2", function(d) { return xScale(d.upperBound); })
		.attr("y2", function(d) { return yScale(d.plotNum); })
        .style("stroke", function(d) {
            return orderColor(d.lowerBound, param, d.upperBound);
        })
        .style("stroke-width", "2px")
...
STAP.UIHandlers =
{
    ...
    // Get the property from the selected element
    getStyleProperty: function(id, propName)
    {
        var elt = document.getElementById(id);
        if (elt) return elt.style[propName];
    },
    ...
    extractElementClassValue: function(elt, cls, setDefaultValue, bPlaceholderFallback)
    {
        ...
        if (ctl)
        {
            ...
            if (ctl.value)
                return ctl.value;
            else if (bPlaceholderFallback && ctl.placeholder)
    ...
    pasteFromExcelHandler: function(e)
    {
        ...
        var obj = (e.clipboardData || window.clipboardData);
        if (obj)
        {
            var str = obj.getData('text');
            str = str.replaceAll(/[\r\n\t]+/g, " "); // replace carriage return, newline or tab sequence by a single space
            
            // Replace hyphens, emdashes, endashes
            ...
            else
            {
                ...
                if (context.selectionStart !== undefined)
                    context.value = context.value.slice(0, context.selectionStart) + str + context.value.slice(context.selectionEnd);
                e.preventDefault();
            }
        }
    },
    ...
    HTTPRequest: function(isGet, url, queryString, fnSucc, fnFail, fnError)
    {
        ...
        xhttp.onreadystatechange = function()
        {
            if (this.readyState == 4)
            {
                ...
                if (this.status == 200)
                    if (fnSucc) fnSucc(STAP.Utility.trimString(this.responseText));
                else
    ...
    /* Functions for temporary message setters */
    
    _tmpMsgWorkers: {},
    
    tempSetMessage: function(spnId, msg, timems, bIsErr)
    {
        if (this._tmpMsgWorkers[spnId])
        {
            clearTimeout(this._tmpMsgWorkers[spnId]);
            delete this._tmpMsgWorkers[spnId];
        }
        UI.setProperty(spnId,"innerHTML",msg);
        UI.setProperty(spnId,"className",(bIsErr ? "errormsg" : "successmsg"));
        
        var self = this;
        this._tmpMsgWorkers[spnId] = setTimeout(function() {
            UI.setProperty(spnId,"innerHTML","");
            UI.setProperty(spnId,"className","");
            delete self._tmpMsgWorkers[spnId];
        }, timems);
    }
};
...
{
    this.containerID = containerID;
    this.SVGTree = document.createDocumentFragment();
};
...
STAP.HTMLStemplot.prototype.stemplot = function(data1, name1, varname, splitLevel, sigAdj, gapHide, data2, name2)
{
    ...
    if (this.containerID)
    {
        ...
        elt.innerHTML = tblStr;

        elt = document.querySelector(".stemplotTable");

        if (this.SVGTree.childElementCount == 0)
            this.SVGTree.appendChild(document.createElementNS("http://www.w3.org/2000/svg", "svg"));

        var svgelt = this.SVGTree.firstElementChild;
//        svgelt.setAttribute("viewbox", "0 0 " + elt.clientWidth + " " + elt.clientHeight);
        var sourcestring = '<style> div{color:black; font: 12px;} ' +
        '.stemplotTable { border: none; }'+
'.stemplotHeader {text-align: center; font-weight: bold; border: none; }'+
'.stemplotFooter {text-align: center; font-weight: bold; border: none; white-space: nowrap; } ' +
'.stemplotVar1 { text-align: right; font-weight: bold; border: none; }' +
'.stemplotVar2 { text-align: left; font-weight: bold; border: none; }' +
'.stemplotCenterStem { text-align: center; font-weight: bold; font-family: "Courier New", Courier, monospace; border-top: none; border-bottom: none;    border-left: 1px solid black; border-right: 1px solid black; white-space: nowrap; width: 1px; }' +
'.stemplotLeftStem { text-align: right; font-weight: bold; font-family: "Courier New", Courier, monospace; border-top: none; border-bottom: none; border-left: none; border-right: 1px solid black; white-space: nowrap; }' +
'.stemplotRightAlign { text-align: right; font-family: "Courier New", Courier, monospace; border: none; white-space: nowrap; }' +
'.stemplotLeftAlign { text-align: left; font-family: "Courier New", Courier, monospace; border: none; white-space: nowrap; }</style>' +
    '<polygon fill="white" points="0,0 ' + elt.clientWidth + ',0 ' + elt.clientWidth + ',' + elt.clientHeight + ' 0,' + elt.clientHeight + '" />' +
        '<foreignObject x="0" y="0" width="' + elt.clientWidth + '" height="' + elt.clientHeight + '"><div xmlns="http://www.w3.org/1999/xhtml">' +
        tblStr + '</foreignObject>';
        svgelt.innerHTML = sourcestring;
    }
};
...
STAP.InputValidation = 
{
	...
	else
	{
        ...
        else if (spnMsg && strMeasure == "percent" && Math.abs(val) < 0.01)
            spnMsg.innerHTML =
...
STAP.Statistics =
{
    ...
    onePropZTest: function(x, n, p0, sidedness)
    {
        ...
        return {
            z: zScore,
            pValue: (sidedness === STAP.Statistics.ONE_SIDED_LT ? leftTail :
                    (sidedness === STAP.Statistics.ONE_SIDED_GT ? 1 - leftTail : 2 * Math.min(leftTail, 1-leftTail))),
            pHat: pHat
        };
    },
    ...
    // Return the parameters for a z-confidence interval for a sample mean, given the sample mean and population SD
    oneSampZIntervalMeanStats: function(mean, popSD, n, cLevel)
    {
        var SEM = popSD / Math.sqrt(n);
        var zStar = jStat.normal.inv((1 - cLevel) / 2, 0, 1);
        return {
            lowerBound: mean + zStar * SEM,
            upperBound: mean - zStar * SEM
        };
    },

    // Return the parameters for a chi-square confidence interval for a sample SD, given the sample data array
    oneSampX2IntervalSD: function(arr, cLevel)
    {
        return STAP.Statistics.oneSampX2IntervalSDStats
        (jStat.stdev(arr, true), arr.length, cLevel);
    },
    
    // Return the parameters for a chi-square confidence interval for a sample SD, given the sample statistics
    oneSampX2IntervalSDStats: function(SD, n, cLevel)
    {
        var tail = (1 - cLevel) / 2;
        var df = n - 1;
        var XL2 = jStat.chisquare.inv(tail, df);
        var XU2 = jStat.chisquare.inv(1 - tail, df);
        return {
            lowerBound: Math.sqrt(df * SD * SD / XU2),
            upperBound: Math.sqrt(df * SD * SD / XL2),
            df: df
        };
    },
    ...
    // Return the parameters from a X2-test for a sample SD, given the sample data array and the null hypothesis SD    
    oneSampX2TestSD: function(arr, SD, sidedness)
    {
        return this.oneSampX2TestSDStats(jStat.stdev(arr, true), arr.length, SD, sidedness);
    },

    // Return the parameters from a X2-test for a sample SD, given the sample statistics and the null hypothesis SD    
    oneSampX2TestSDStats: function(Sx, n, SD, sidedness)
    {
        var df = n - 1;
        var X2 = df * Sx * Sx / (SD * SD);
        var leftTail = jStat.chisquare.cdf(X2, df);
        return {
            X2: X2,
            pValue: (sidedness === STAP.Statistics.ONE_SIDED_LT ? leftTail :
                    (sidedness === STAP.Statistics.ONE_SIDED_GT ? 1 - leftTail : 2 * Math.min(leftTail, 1-leftTail))),
    ...
    // Return the parameters from an F-test for a difference in variances, given the sample standard deviations
    // Null hyp diff = 0
    twoSampFTestDiffVarStats: function(s1, s2, n1, n2, sidedness)
    {
        var sL = Math.max(s1, s2);
        var sS = Math.min(s1, s2);
        var nL = (STAP.SafeNumber.compareToWithinTolerance(sL, s1) == 0 ? n1 : n2);
        var nS = ((nL == n1) ? n2 : n1);
        var F = (sL * sL) / (sS * sS);
        
        var rightTail = 1 - jStat.centralF.cdf(F, nL - 1, nS - 1);
        return {
            F: F,
            pValue: (sidedness === STAP.Statistics.TWO_SIDED ? 2 * rightTail : rightTail),
            dfNum: nL - 1,
            dfDenom: nS - 1
        };
    },
    ...
    // Return the parameters for the mean y value at a given x for a linear regrssion given the sample data
    linRegMeanYInterval: function(xArr, yArr, x, cLevel)
    {
        var n = xArr.length;
        var reg = this.polynomialRegression(xArr, yArr, 1);
        var xbar = jStat.mean(xArr);
        var sx = jStat.stdev(xArr, true);
        var SE = reg.S * Math.sqrt(1 / n + Math.pow(x - xbar, 2)/(n - 1)/(sx * sx));
        var tStar = jStat.studentt.inv((1 - cLevel) / 2, n - 2);
        var ptEst = reg.coeffs[0] + x * reg.coeffs[1];
        return {
            lowerBound: ptEst + tStar * SE,    // tStar is negative
            upperBound: ptEst - tStar * SE,
            df: n - 2
    ...
    // Return the parameters for a y-prediction interval for regression slope given the sample data
    linRegYPredictionInterval: function(xArr, yArr, x, cLevel)
    {
        var n = xArr.length;
        var reg = this.polynomialRegression(xArr, yArr, 1);
        var xbar = jStat.mean(xArr);
        var sx = jStat.stdev(xArr, true);
        var SE = reg.S * Math.sqrt(1 + 1 / n + Math.pow(x - xbar, 2)/(n - 1)/(sx * sx));
        var tStar = jStat.studentt.inv((1 - cLevel) / 2, n - 2);
        var ptEst = reg.coeffs[0] + x * reg.coeffs[1];
        return {
            lowerBound: ptEst + tStar * SE,    // tStar is negative
    ...
    polynomialRegression: function(arrX, arrY, order)
    {
        ...
        var mtminv = jStat.inv(jStat.multiply(mt,m));
        
        // Following algorithm taken from Elementary Linear Algebra, Anton 10/e p. 473
        var coeffs = jStat.transpose(jStat.multiply(jStat.multiply(mtminv,mt),y));
        ...
        var S = ((arrY.length == order + 1) ? 0 : Math.sqrt(sumSqrResid / (arrY.length - (order + 1))));
        ...
        if (order == 1)
        {
            retval.SEintercept = S * Math.sqrt(mtminv[0][0]);
            retval.SEslope = S * Math.sqrt(mtminv[1][1]);
            retval.Tintercept = coeffs[0]/retval.SEintercept;
            retval.Tslope = coeffs[1]/retval.SEslope;
            retval.Pintercept = 2 * jStat.studentt.cdf(-Math.abs(retval.Tintercept),arrX.length - 2);
            retval.Pslope = 2 * jStat.studentt.cdf(-Math.abs(retval.Tslope),arrX.length - 2);
        }
	...
	// arrX[m][n-1] is the value of the mth explanatory variable for the nth data point
        ...
        // Following algorithm taken from Elementary Linear Algebra, Anton 10/e p. 473
        var coeffs = (arrX.length > 0) ? jStat.transpose(jStat.multiply(jStat.multiply(mtminv,mt),y))
       					: [jStat.mean(arrY)];
	...
	// Evaluate the model response at the data point with index n
        ...
        var df = (arrY.length - (arrX.length + 1));
        var S = Math.sqrt(sumSqrResid / df);
        var SEs = new Array(coeffs.length);
        var Ts = new Array(coeffs.length);
        var Ps = new Array(coeffs.length);
        for (var i = 0; i < SEs.length; i++)
        {
            SEs[i] = S * Math.sqrt(mtminv[i][i]);
            Ts[i] = coeffs[i] / SEs[i];
            Ps[i] = 2 * jStat.studentt.cdf(-Math.abs(Ts[i]),df);
        }
        
        var ANOVA = {
            dfReg: arrX.length,
            dfError: df,
            dfTotal: arrY.length - 1,
            SSReg: SST - sumSqrResid,
            SSError: sumSqrResid,
            SSTotal: SST
        };
        
        ANOVA.MSReg = ANOVA.SSReg/ANOVA.dfReg;
        ANOVA.MSError = ANOVA.SSError/ANOVA.dfError;
        ANOVA.F = ANOVA.MSReg/ANOVA.MSError;
        ANOVA.P = jStat.ftest(ANOVA.F,ANOVA.dfReg,ANOVA.dfError);
        
        return {
            coeffs: coeffs,
            residuals: residuals,
            RSquared: SSreg/SST,
            RSquaredAdj: 1 - (S * S) / Math.pow(jStat.stdev(arrY, true), 2),
            S: S,
            SEs: SEs,
            Ts: Ts,
            Ps: Ps,
    ...
    oneWayANOVA: function(dataArrArr, followUp, followUpCLevel)
    {
        var means = [];
        var SDs = [];
        var ns = [];
        dataArrArr.forEach(function(arr) {
            means.push(jStat.mean(arr));
            SDs.push(jStat.stdev(arr, true));
            ns.push(arr.length);
        });
        return STAP.Statistics.oneWayANOVAStats(means, SDs, ns, followUp, followUpCLevel);

    /* This implementation seems broken for n = 2?
       var N = 0;
    ...
    oneWayANOVAStats: function(means, SDs, ns, followUp, followUpCLevel)
    {
    	var N = 0;
    	...
    	var dfb = means.length - 1;
    	var dfw = N - means.length;
    	
    	var F = (ssb / ssw) * (dfw /dfb);
    	var retval = {};
    	retval.F = F;
    	retval.ssb = ssb;
    	retval.ssw = ssw;
    	retval.pValue = jStat.ftest(F, dfb, dfw);
    	retval.dfb = dfb;
       	retval.dfw = dfw;
       	retval.msb = ssb / dfb;
       	retval.msw = ssw / dfw;
        ...
        if (followUp)
        {
            ...
            while (firstGroup < (means.length - 1))
            {
                while (secondGroup < means.length)
                {
                    var LSD = -jStat.studentt.inv((1 - followUpCLevel) / 2, dfw) * Math.sqrt(retval.msw * (1 / ns[firstGroup] + 1 / ns[secondGroup]));
                    var meandiff = means[firstGroup] - means[secondGroup];
                    
                    retval.LSDLabels.push("Group " + (firstGroup + 1) + " - Group " + (secondGroup + 1));
                    retval.LSDIntervals.push(
                    {
                        lowerBound: meandiff - LSD,
                        upperBound: meandiff + LSD
                    });
                    secondGroup++;
                }
                firstGroup++;
    ...
    signTest: function(diffArr, sidedness)
    {
        var negDiff = 0, posDiff = 0;
        for (var i = 0; i < diffArr.length; i++)
            if (STAP.SafeNumber.compareToWithinTolerance(diffArr[i], 0) < 0)
                negDiff++;
            else if (STAP.SafeNumber.compareToWithinTolerance(diffArr[i], 0) > 0)
                posDiff++;
                
        var x = (sidedness == STAP.Statistics.ONE_SIDED_LT ? negDiff :
                (sidedness == STAP.Statistics.ONE_SIDED_GT ? posDiff :
                    Math.max(negDiff, posDiff)));

        var pValue = 1 - jStat.binomial.cdf(x - 1, negDiff + posDiff, 0.5);
        if (sidedness == STAP.Statistics.TWO_SIDED) pValue *= 2;
        return {
            X: x,
            pValue: pValue
        };
    ...
    medianSignTest: function(arr, median, sidedness)
    {
        return this.signTest(arr.map(function(x){return x - median;}), sidedness);
    },
    
    nonParametricRanks: function(arrArr)
    {
        var rawList = [];
        for (var i = 0; i < arrArr.length; i++)
            arrArr[i].forEach(x => {rawList.push({
                value: x,
                group: i + 1
                });});
        if (rawList.length == 0) return [];
        
        STAP.Utility.sortArrayAscendingByProperty(rawList, "value");
        
        var pushQueue = [rawList.shift()];
        var mergedList = [];

        var emptyPushQueue = function()
        {
            // The non-averaged ranks of the k identical items to be pushed are
            // (itemsPushed + 1), (itemsPushed + 2), ..., (itemsPushed + k)
            var avgRank = mergedList.length + (1 + pushQueue.length) / 2;
        ...
        {
            // If there is something in the push queue that does not match
            // the next item to be processed, empty the push queue
            // into the final merged list since you know it is safe to do so.
            //
            // Then, push this item into the queue. In this way, the push
            // queue only contains identical items, and will contain at least
            // the most recently examined item.
            var nextItem = rawList.shift();
            if (!STAP.SafeNumber.isZeroWithinTolerance(pushQueue[0].value - nextItem.value))
                emptyPushQueue();
            pushQueue.push(nextItem);
        }
        emptyPushQueue();
        
        // now, reseparate the groups
        var ranks = [];
        for (var i = 0; i < arrArr.length; i++) ranks.push([]);
        
        while (mergedList.length > 0)
        {
            var item = mergedList.shift();
            ranks[item.group - 1].push(item.rank);
        }
        
        return ranks;
    },

    wilcoxonSignedRankTest: function(diffArr, sidedness)
    {
        var n = diffArr.length;
        var matches = 0;
        var negDiff = [], posDiff = [];
        for (var i = 0; i < n; i++)
        {
            if (STAP.SafeNumber.compareToWithinTolerance(diffArr[i], 0) < 0)
                negDiff.push(-diffArr[i]);
            else if (STAP.SafeNumber.compareToWithinTolerance(diffArr[i], 0) > 0)
                posDiff.push(diffArr[i]);
            else
                matches++;
        }
        n -= matches;
        
        var ranks = this.nonParametricRanks([negDiff, posDiff]);
        var negSum = jStat.sum(ranks[0]), posSum = jStat.sum(ranks[1]);
        var W = Math.min(negSum, posSum);

        var twoTailed5 = [-1,0,2,3,5,8,10,13,17,21,25,30,35,40,46,52,58,66,73,81,89,98,107,116,126,137];
        var twoTailed1 = [-1,-1,-1,0,1,3,5,7,10,13,16,19,23,27,32,37,42,48,54,61,68,75,83,91,100,109];
        var oneTailed5 = [0,2,3,5,8,10,13,17,21,25,30,35,41,47,53,60,67,75,83,91,100,110,119,130,140,151];
        var oneTailed1 = [-1,-1,0,1,3,5,7,10,12,16,19,23,28,33,37,43,49,55,62,69,76,84,93,101,110,120];
        
        var lookupPValueHTML = function(w)
        {
            if (n < 5) return "&gt;0.05"
            var tableIndex = n - 5;
            var crit1 = (sidedness == STAP.Statistics.TWO_SIDED ? twoTailed1 : oneTailed1)[tableIndex];
            var crit5 = (sidedness == STAP.Statistics.TWO_SIDED ? twoTailed5 : oneTailed5)[tableIndex];
        ...
        var normalApprox = function()
        {
            var mean = n * (n + 1) / 4;
            var SD = Math.sqrt(n * (n + 1) * (2 * n + 1) / 24);
            
            if (sidedness == STAP.Statistics.TWO_SIDED)
                return 2 * jStat.normal.cdf((Math.min(negSum, posSum) - mean) / SD, 0, 1);
            else return 1 - jStat.normal.cdf(
                ((sidedness == STAP.Statistics.ONE_SIDED_GT ? posSum : negSum) - mean) / SD, 0, 1);
        };
        ...
        return {
            ...
            Wminus: negSum,
            Wplus: posSum,
            pValueHTML: (n > 30 ? STAP.Format.formatPValueHTML(normalApprox()) : lookupPValueHTML(W))
        };
    },

    medianWilcoxonSignedRankTest: function(arr, median, sidedness)
    {
        return this.wilcoxonSignedRankTest(arr.map(function(x){return x - median;}), sidedness);
    },
    
    wilcoxonRankSumApproxTest: function(arr1, arr2, sidedness)
    {
        var ranks = this.nonParametricRanks([arr1, arr2]);
        var ranks1 = ranks[0], ranks2 = ranks[1];
        
        var R1 = jStat.sum(ranks1);
        var mu1 = ranks1.length * (ranks1.length + ranks2.length + 1) / 2;
        var sigma1 = Math.sqrt(mu1 * ranks2.length / 6);
        
        var z = (R1 - mu1) / sigma1;
        var leftTail = jStat.normal.cdf(z, 0, 1);
    ...
    kruskalWallisTest: function(arrArr)
    {
        var ranks = this.nonParametricRanks(arrArr);
        var N = 0, subsum = 0;
        for (var i = 0; i < ranks.length; i++)
        {
            var n = ranks[i].length;
            N += n;
            var R = jStat.sum(ranks[i]);
            subsum += R * R / n;
        }
        var H = 12 / N / (N + 1) * subsum - 3 * (N + 1);
        var df = arrArr.length - 1;
