...
$ignorecache = (isset($_GET['i']) ? (substr($_GET['i'], 0, 1) == '1') : false);
...
if (!$ignorecache && $data = $Cache_Lite->get($classcode))
{
    echo $data;
    ...
    if ($tblPS->rowCount() > 0)
    {
        ...
        if (!$is2var)
        {
            ...
            $adjstr = "," . $tblparams["Var2Name"];

        $varind = ($is2var ? 2 : 1);
        $pgstr = $tblparams["Enabled"] . $varind . $tblparams["Var1Name"] . $adjstr . "\n";
        ...
        if ($is2var)
            ...
            foreach ($dataPS as $row)
            {
                ...
                $pgstr .= ($grpid . "," . $row["Var1"] . ($isquant ? "" : "," . $row["Frequency"]) . "\n");
            }
