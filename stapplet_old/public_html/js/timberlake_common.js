...
function handleSingerChange()
{
    var abbr = UI.getProperty("selSingerName", "value");
    UI.setStyleProperty("spnSingerName" + abbr, "display", "inline");
    UI.setStyleProperty("divIntro" + abbr, "display", "block");
    document.title = (abbr == "JT" ? "Justin Timberlake " : "Taylor Swift ") +
        " Activity, part 1";   
    
    var players = document.querySelectorAll("div.crowdbkg > lottie-player");
    
    _spec = function(s1, s2) {
        UI.batchSetStyleProperty(["divIntro" + s1, "spnSingerName" + s1], "display", "none");
        players.forEach((play) => {play.load("./json/" + s2 + "_whole_crowd.json")});
    };
...
{
    UI.batchSetStyleProperty(["divIntro", "divShowResults", "divCodeData", "divOfflineAdmin"], "display", "none");
...
function _manySamples(fn2, fn3)
{
    ...
    if (howMany > 2000)
    {
        UI.tempSetMessage("spnLastEditMsg", "Only 2000 items can be added at a time.", 2000, true);
        return;
    }
    ...
    UI.tempSetMessage("spnLastEditMsg", "Data generated.", 2000);
...
function revealRestart()
{
    ...
    {
        graphs[k].setMarker(trueMean, "True Mean = 78.38", "#990000");
...
function addBatchData()
{
    if (IV.validateInputFloatArray("txtDataBatchAdd", "spnLastEditMsg", "Data"))
    {
        ...
        if (confirm("Are you sure? This operation cannot be undone."))
        {
            ...
            UI.tempSetMessage("spnLastEditMsg", "Data added to plot.", 2000);
            refreshData();
        }
    }
}
