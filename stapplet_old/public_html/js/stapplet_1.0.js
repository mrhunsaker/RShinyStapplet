...
/** SVGGraph */
STAP.SVGGraph = function(containerID, width, height)
{
	if (!width) width = 600;
...
STAP.SVGGraph.prototype.dotplot = function(data, xAttr, caseAttr, bAllowCount, bIsRelative)
{
	var self = this;
	var dataMap = {};
	...
	var dot_buffer = 0.02 * (data[data.length - 1].dataValue - data[0].dataValue);
	...
	var diam = Math.max(xScale(wilk_h) - xScale(0), 0.9 * (xScale(min_diff) - xScale(0)));
...
STAP.SVGGraph.prototype.parallelDotplot = function(dataArr, groupNameArr, xAttr, caseAttr)
{
	...
	if (diam * (global_max_count + 2) < 0.9 * yOffset)
	{
		...
		this.height = yOffset * dataArr.length + graph_margin * (dataArr.length - 1);
		d3.select("#" + this.svgID)
			.attr("height", "" + (this.height + this.margin.top + this.margin.bottom));
	}
...
STAP.SVGGraph.prototype.barChart = function(data, xAttr, bRelative)
{
	...
	var verticaloffset = parseInt(STAP.Preferences.getPreference(STAP.Preferences.keys.drawing.font_size)) / 2 - 2;
	  ...
	  this.svg.selectAll(".fillable")
		...
		var pref = STAP.Preferences;
		  d3.select(this)
	  		.style("fill", d3.rgb(pref.getPreference(pref.keys.drawing.fill_color)).brighter(0.4).toString());
		  self.tooltip.transition()
		       .duration(200)
		       .style("opacity", .9);
		...
		.on("mouseout", function(d) {
		  var pref = STAP.Preferences;
		  d3.select(this)
	  		.style("fill", pref.getPreference(pref.keys.drawing.fill_color));
		  self.tooltip.transition()
		       .duration(500)
		       .style("opacity", 0);
		});	
...
STAP.SVGGraph.prototype.scatterplot = function(data, xAttr, yAttr, caseAttr, dotRadius)
{
	...
	// do not want dots overlapping axis, so add in buffer to data domain
	var xMin = d3.min(data, function(d) { return d[xAttr]; });
	var xMax = d3.max(data, function(d) { return d[xAttr]; });
	var yMin = d3.min(data, function(d) { return d[yAttr]; });
	var yMax = d3.max(data, function(d) { return d[yAttr]; });
...
STAP.SVGGraph.prototype.fnRangeMap = function(fn, L, R, n)
{
...
STAP.SVGGraph.prototype.plotTopCurve = function(fn, n, bNoClear)
{
	var self = this;
