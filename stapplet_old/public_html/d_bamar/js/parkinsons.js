...
fnInitializeBegin = function()
{
    UI.batchSetStyleProperty(["txtGraphDataCSV", "divIntro", "divGraphAdmin",
            "divActivity", "divActivityResults", "divResults", "divOfflineAdmin", "divCodeData"], "display", "none");
...
function doManyOfflineSamples()
{
    ...
    if (numSamples > 2000)
    {
        tempSetMessage("spnOfflineEditMsg", "You can only add 2000 items at a time.", 2000, true);
        return;
    }
    for (var i = 0; i < numSamples; i++)
    {
...
{
    resetIdle();
    file.saveCSV("txtGraphDataCSV", "parkinsons_data" +
        (offline ? "" : "_" + classCode));
}
