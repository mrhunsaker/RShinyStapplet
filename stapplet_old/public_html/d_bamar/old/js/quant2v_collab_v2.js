...
fnDataSyncSuccess = function(bChanged)
{
    if (bChanged)
    {
        ...
        {
            graphData[0] = rawData[0].slice(0);
            graphData[1] = rawData[1].slice(0);
        }
        else
        {
            graphData[0] = [];
...
function addSingleData()
{
    ...
    graphData[0].push(lastAdd.x);
    graphData[1].push(lastAdd.y);
...
function updateResidualPlot(stats)
{
    ...
    residualPlot.scatterplot(util.arraysTo2DGraphData(graphData[0],
...
function updateModel()
{
    if (graphData[0].length > 0)
    {
        ...
        graphDataArr = util.arraysTo2DGraphData(graphData[0], graphData[1],
...
function updateLSRL()
{
    var stat1 = stat.polynomialRegression(graphData[0], graphData[1], 1);
    ...
    tableHTML += "<TD><em>y&#770;</em> = " + util.polynomialRegEQDisplayHTML(stat1.coeffs) + "</TD><TD>"
        + graphData[0].length + "</TD><TD>"
        + format.formatNumber(stat1.S) + "</TD><TD>"
        + format.formatNumber(jStat.corrcoeff(graphData[0], graphData[1])) + "</TD><TD>"
...
function updateHigherOrderModel()
{
    ...
    if (modelType == "quadratic" && graphData[0].length < 3)
    {
    ...
    var stat1 = (modelType == "quadratic" ? stat.polynomialRegression(graphData[0], graphData[1], 2) :
        stat.exponentialRegression(graphData[0], graphData[1]));
    ...
    tableHTML += "</TD><TD>" + graphData[0].length + "</TD><TD>"
        + format.formatNumber(stat1.S) + "</TD><TD>"
        + format.formatNumber(stat1.rSquared) + "</TD></TR></TABLE>";
    UI.setProperty("spnModelResults", "innerHTML", tableHTML);
    updateResidualPlot(stat1);
