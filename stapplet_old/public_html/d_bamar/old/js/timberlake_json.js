...
// 0 = SRS, 1 = Stratify row, 2 = Stratify column
...
function _writeBlank(tbl, zeroRowInd, zeroColInd)
{
    // HTML row 0 is the Timberlake picture
    tbl.rows[zeroRowInd + 1].cells[zeroColInd].innerHTML = '<lottie-player src="json/timberlake_crowd_member.json" background="transparent"  speed="1" style="width: 30px; height: 30px;" loop autoplay></lottie-player>';    
}

function _writeSelected(tbl, zeroRowInd, zeroColInd)
{
    // HTML row 0 is the Timberlake picture
    if (showInstant)
    {
        var enjoyment = realData[zeroRowInd][zeroColInd];
        tbl.rows[zeroRowInd + 1].cells[zeroColInd].innerHTML = '<lottie-player src="json/timberlake_crowd_custom.php?e=' + enjoyment + '" background="transparent"  speed="1" style="width: 30px; height: 30px;" loop autoplay></lottie-player>'  + (showInstant ? '<span class="textoverlay">' + enjoyment + '</span>' : '');
    }
    else
        tbl.rows[zeroRowInd + 1].cells[zeroColInd].innerHTML = '<lottie-player src="json/timberlake_crowd_selected.json" background="transparent"  speed="1" style="width: 30px; height: 30px;" loop autoplay></lottie-player>';
}

function clearTable(groupInd, bInsureFull)
{
    if (prevSamples[groupInd])
    {
        var tbl = document.getElementById(tblIds[groupInd]);
        for (var i = 0; i < prevSamples[groupInd].length; i++)
        {
            var ind = getZeroIndices(prevSamples[groupInd][i]);
            _writeBlank(tbl, ind.row, ind.col);
        }
        prevSamples[groupInd] = null;
    }
    else if (bInsureFull)
        _clearFullTable(tblIds[groupInd]);
}

function _clearFullTable(tblId)
{
...
var prevSamples = [null, null, null];
...
function writeIntoTable(groupInd)
{
    ...
    for (var i = 0; i < arr.length; i++)
    {
        ...
        _writeSelected(tbl, ind.row, ind.col);
    }
    prevSamples[groupInd] = samples[groupInd].slice(0);
}
...
function generateStratifyRow()
{
    ...
    for (var i = 0; i < 5; i++)
    {
        ...
        samples[1].push(shuffler10[0] + 10 * i);
        samples[1].push(shuffler10[1] + 10 * i);
    }
    _generateHelper(1);
}
...
function generateStratifyColumn()
{
    samples[2] = [];
    for (var i = 0; i < 10; i++)
        samples[2].push(i + 10 * util.randomIntFromInterval(0,4));
    _generateHelper(2);
}
