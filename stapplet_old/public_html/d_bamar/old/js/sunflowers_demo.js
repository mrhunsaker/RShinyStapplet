...
function _doStratifyRow()
{
    var arr = [];
    for (var i = 0; i < 10; i++)
    {
        util.knuthShuffle(shuffler10);
        arr.push(shuffler10[0] + 10 * i);
    }
    return arr;
}
...
{
    samples[1] = _doStratifyRow();
    _generateHelper(1);
}

function _doStratifyColumn()
{
    var arr = [];
    for (var i = 0; i < 10; i++)
        arr.push(i + 10 * util.randomIntFromInterval(0,9));
    return arr;
}

function generateStratifyColumn()
{
    samples[2] = _doStratifyColumn();
    _generateHelper(2);
}
...
{
    if (simpleValidate("txtDataBatchAdd", "spnDataBatchAddMsg", "Number of trials", parseInt))
    {
        var means = [];
        var groupNum = UI.getProperty("selPlotEdit", "value");
        var numTrials = parseInt(UI.getProperty("txtDataBatchAdd", "value"));

        for (var i = 0; i < numTrials; i++)
        {
            if (groupNum == "1")
                means.push(getSampleMean(_doSRS()));
            else if (groupNum == "2")
                means.push(getSampleMean(_doStratifyRow()));
            else if (groupNum == "3")
                means.push(getSampleMean(_doStratifyColumn()));
        }
    
        var datastr = JSON.stringify(means);
