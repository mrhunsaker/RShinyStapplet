...
function addCountsTableRow()
{
    ...
    + (table.rows.length - 2) + '" onChange="return deactivatePage(this)" onFocus="handleTableEntryFocus(this)" onBlur="handleTableEntryBlur(this,\'[category name]\')" VALUE="[category name]" SIZE="15"></TD>';
    for (var i = 0; i < numDataColumns; i++)
        row.insertCell(i + 1).innerHTML = '<TD><INPUT TYPE="text" TITLE="Data" CLASS="data" ID="txtCountsTblData'
    + (table.rows.length - 2) + '_' + (i + 1) + '" onChange="return deactivatePage(this)" onFocus="handleTableEntryFocus(this)" onBlur="handleTableEntryBlur(this,\'[frequency]\')" VALUE="[frequency]" SIZE="15"></TD>';

    // Fix the rowspan of the rightmost column (single add button) 
...
function addCountsTableColumn()
{
    ...
    table.rows[0].insertCell(numDataColumns + 1).innerHTML = '<TD><INPUT TYPE="text" TITLE="Column Name" CLASS="colName" ID="txtCountsTblColName'
        + (numDataColumns + 1) + '" onChange="return deactivatePage(this)" onFocus="handleTableEntryFocus(this)" onBlur="handleTableEntryBlur(this,\'[group name]\')"  VALUE="[group name]" SIZE="15"></TD>';
    ...
    for (var i = 1; i < table.rows.length - 1; i++)
        ...
        + i + '_' + (numDataColumns + 1) + '" onChange="return deactivatePage(this)" onFocus="handleTableEntryFocus(this)" onBlur="handleTableEntryBlur(this,\'[frequency]\')" VALUE="[frequency]" SIZE="15"></TD>';
...
function removeCountsTableRow()
{
    ...
    // Fix the rowspan of the right column
    document.getElementById("tdRowspanInner").rowSpan--;
}
...
function removeCountsTableColumn()
{
    ...
    table.rows[0].deleteCell(numDataColumns);
...
function validateInputBuildData()
{
    ...
    if (UI.getProperty("selInputType", "value") == "counts")
    {
        ...
        var columnNames = [];
        for (var i = 1; i < rows[0].cells.length - 1; i++)               
            columnNames.push(UI.extractElementClassValue(rows[0].cells[i], "colName", true));
...
function rebuildTable()
{
    ...
    table.rows[0].cells[1].getElementsByTagName("input")[0].value = "[group name]";
    table.rows[0].cells[1].getElementsByTagName("input")[0].id = "txtCountsTblColName1";
    table.rows[0].cells[2].getElementsByTagName("input")[0].value = "[group name]";
    table.rows[0].cells[1].getElementsByTagName("input")[0].id = "txtCountsTblColName2";
    while (table.rows[0].cells.length > 4) table.rows[0].deleteCell(3);
    table.rows[1].cells[0].getElementsByTagName("input")[0].value = "[category name]";
    table.rows[1].cells[0].getElementsByTagName("input")[0].id = "txtCountsTblRowName1";
    table.rows[1].cells[1].getElementsByTagName("input")[0].value = "[frequency]";
    table.rows[1].cells[1].getElementsByTagName("input")[0].id = "txtCountsTblData1_1";
    table.rows[1].cells[2].getElementsByTagName("input")[0].value = "[frequency]";
    ...
    table.rows[2].cells[0].getElementsByTagName("input")[0].value = "[category name]";
    table.rows[2].cells[0].getElementsByTagName("input")[0].id = "txtCountsTblRowName2";
    table.rows[2].cells[1].getElementsByTagName("input")[0].value = "[frequency]";
    table.rows[2].cells[1].getElementsByTagName("input")[0].id = "txtCountsTblData2_1";
    table.rows[2].cells[2].getElementsByTagName("input")[0].value = "[frequency]";
...
function handle2CatInferenceChange(sel)
{
    ...
    {
        UI.setStyleProperty("spn2CatChiOptions", "display", "none");
    ...
    else // chi-squared
    {
        UI.batchSetStyleProperty(["spnZIntervalOptions", "spnZTestOptions", "spnSuccessCat", "spnSimulationOptions"],
                    "display", "none");
        UI.setStyleProperty("spn2CatChiOptions", "display", "inline");
    }                            
}
...
function updateGraph()
{
    ...
    if (graphType == "side")
    {
        ...
        graphContainer.setGraph(new SPAApplet.SideBySideBarGraph(data, "Group", varname));
    }
    else
    {
        UI.setStyleProperty("spnSideBarGraphOptions", "display", "none");
        graphContainer.setGraph(new SPAApplet.StackedBarGraph(data, "Group", varname));
    }
}
                ...
                Number.POSITIVE_INFINITY, false, "spnSimulationErrorMsg", "Number of samples", "must be positive."))
            {
                ...
                if (!simulationGraphContainer)
                    simulationGraphContainer = new SPAApplet.QuantitativeGraphContainer("cnvSimulationPlot",
                                                "Simulated difference in sample proportion" + (successCat.length > 0 ? " of " + successCat : ""));
                else
                {
                    simulationGraphContainer.setVariableName("Simulated difference in sample proportion" +
                                                                    (successCat.length > 0 ? " of " + successCat : ""));
...
{
    file.saveCanvas("cnvSimulationPlot", "simulation_diffprop");
}
...
function initializePageMultiGrp()
{
