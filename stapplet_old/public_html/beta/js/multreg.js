...
var lastPredictorIndices = [];

function validateInput()
{
    lastPredictorIndices = [];
    xDataArrArr = [];
    yDataArr = null;
...
function handleRegression()
{
	...
	resultHTML += "<BR>S = " + format.formatNumber(reg.S) + "&nbsp;&nbsp;&nbsp;R&sup2; = " + format.formatNumber(reg.RSquared) + "&nbsp;&nbsp;&nbsp;R&sup2;(adj) = " + format.formatNumber(reg.RSquaredAdj);
	
	resultHTML += "<BR><BR><TABLE CLASS='results'><TR>";
    resultHTML += "<TH>Predictor</TH><TH>Coef</TH><TH>SE Coef</TH><TH>T</TH><TH>P</TH></TR><TR>";
    resultHTML += "<TD>Constant</TD><TD>" + format.formatNumber(reg.coeffs[0]) + "</TD><TD>" + format.formatNumber(reg.SEs[0]) + "</TD><TD>" + format.formatNumber(reg.Ts[0]) + "</TD><TD>" + format.formatNumber(reg.Ps[0]) + "</TD></TR>";
    for (var i = 1; i < reg.coeffs.length; i++)
    {
		var expName = (fnFetch("txtExplanatory" + lastPredictorIndices[i - 1] + "Name") ? lastFetch : "x<SUB>" + i + "</SUB>");
		resultHTML += "<TR><TD>" + expName + "</TD><TD>" + format.formatNumber(reg.coeffs[i]) + "</TD><TD>" + format.formatNumber(reg.SEs[i]) + "</TD><TD>" + format.formatNumber(reg.Ts[i]) + "</TD><TD>" + format.formatPValueHTML(reg.Ps[i]) + "</TD></TR>";
    }

    resultHTML += "</TABLE><BR><EM>Analysis of Variance</EM></BR><BR><TABLE><TR>";
    resultHTML += "<TH>Source</TH><TH>df</TH><TH>Sum of Squares</TH><TH>Mean Square</TH><TH>F-value</TH><TH>P-value</TH></TR>";
    resultHTML += "<TR><TD>Regression</TD><TD>" +
        format.formatNumber(reg.ANOVA.dfReg) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.SSReg) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.MSReg) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.F) + "</TD><TD>" +
        format.formatPValueHTML(reg.ANOVA.P) + "</TD></TR>";
    resultHTML += "<TR><TD>Error</TD><TD>" +
        format.formatNumber(reg.ANOVA.dfError) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.SSError) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.MSError) + "</TD>" +
        "<TD>&nbsp</TD><TD>&nbsp;</TD></TR>";
    resultHTML += "<TR><TD>Total</TD><TD>" +
        format.formatNumber(reg.ANOVA.dfTotal) + "</TD><TD>" +
        format.formatNumber(reg.ANOVA.SSTotal) + "</TD>" +
