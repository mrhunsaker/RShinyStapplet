  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$populationPlot <- renderPlot({
      ...
      ggplot(df, aes(x = Duration, y = Wait)) +
        ...
        labs(title = "Population: Old Faithful Eruptions",
    ...
    output$samplePlot <- renderPlot({
      ...
      ggplot(df, aes(x = Duration, y = Wait)) +
        ...
        labs(title = "Sample from Population",
    ...
    output$samplingDistPlot <- renderPlot({
      ...
      p <- ggplot(data.frame(Slope = slopes), aes(x = Slope)) +
        ...
        labs(title = "Sampling Distribution of Sample Slopes",
    ...
    output$download_summary <- downloadHandler(
      filename = function() { "oldfaithful_sampling_summary.txt" },
    ...
    output$download_data <- downloadHandler(
      filename = function() { "oldfaithful_sampling_data.csv" },
      content = function(file) {
        write.csv(data.frame(sample_slope = rv$sample_slopes), file, row.names = FALSE)
      }
    )
    output$download_plot <- downloadHandler(
      filename = function() { "oldfaithful_sampling_plot.png" },
      content = function(file) {
    ...
    shinyAccessibility::add_accessibility(
      ...
      role = "main",
      aria_label = "Old Faithful Sampling Distribution Main Panel"
    )
    shinyAccessibility::add_accessibility(
      inputId = ns("sidebarPanel"),
      role = "form",
      aria_label = "Sampling Controls"
    )
    shinyAccessibility::add_accessibility(
